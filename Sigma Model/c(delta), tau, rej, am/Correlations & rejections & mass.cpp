...
int main()
{
  ...
  vector<double> betas = {.5, .75, .825, .9, .925, .95, 1., 1.25, 1.5, 2., 3.};
  vector<double> lambdas = {.3};

  ofstream means, mses, mse_errs, rej, am;
  ...
  am << disc << " discarded," << N << " iterations\n\n";
  means.precision(15);
  mses.precision(15);
  mse_errs.precision(15);
  rej.precision(15);
  am.precision(15);
  ...
  vector<vector<vector<vector<vector<double>>>>> am_means(sizes.size());
  vector<vector<vector<vector<vector<double>>>>> am_mses(sizes.size());
  ...
  for (int size = 0; size < sizes.size(); size ++)
  {
    ...
    am_means[size] = vector<vector<vector<vector<double>>>>(betas.size());
    am_mses[size] = vector<vector<vector<vector<double>>>>(betas.size());
    ...
    for (int β = 0; β < betas.size(); β ++)
    {
      ...
      am_means[size][β] = vector<vector<vector<double>>>(lambdas.size());
      am_mses[size][β] = vector<vector<vector<double>>>(lambdas.size());
      ...
      for (int λ = 0; λ < lambdas.size(); λ ++)
      {
        ...
        c_means[size][β][λ] = vector<vector<double>>(5);
        c_mses[size][β][λ] = vector<vector<vector<double>>>(5);
        c_mse_errs[size][β][λ] = vector<vector<vector<double>>>(5);
        rej_mses[size][β][λ] = vector<double>(K);
        rej_mse_errs[size][β][λ] = vector<double>(K);
        am_means[size][β][λ] = vector<vector<double>>(5);
        am_mses[size][β][λ] = vector<vector<double>>(5);
        for (int i = 0; i < 5; i ++)
        {
          ...
          am_means[size][β][λ][i] = vector<double>(K);
          am_mses[size][β][λ][i] = vector<double>(K);
        ...
        auto start = chrono::steady_clock::now();
        GibbsCorrelation(sigma, N, disc, c_means[size][β][λ], c_mses[size][β][λ], c_mse_errs[size][β][λ], rej_means[size][β][λ], rej_mses[size][β][λ], rej_mse_errs[size][β][λ], am_means[size][β][λ], am_mses[size][β][λ]);
        ...
        am << "d = " << sizes[size].size() << ",L = " << L << ",T = " << T << ",β = " << betas[β] << ",λ = " << lambdas[λ] << "\n";
        for (int i = 0; i < 5; i ++)
        {
          for (int k = 0; k < K; k ++) am << am_means[size][β][λ][i][k] << ",";
          am << "\n";
          for (int k = 0; k < K; k ++) am << am_mses[size][β][λ][i][k] << ",";
          am << "\n";
        }
        am << "\n";
      }
