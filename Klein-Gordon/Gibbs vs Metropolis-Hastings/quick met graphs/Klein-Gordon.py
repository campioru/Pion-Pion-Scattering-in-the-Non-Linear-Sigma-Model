...
εs = [10.**1.5]
ε_strings = [r"10^{1.5}"]
therms = np.empty((len(εs), int(np.ceil(N/10.))))
Bs = 2 ** np.arange(0, K, 1, dtype=int)

times = np.empty(len(εs)+1)
c_means = np.empty((len(εs), 2, L//2 + 1))
c_mses, c_mse_errs = (np.empty((len(εs), 2, L//2 + 1, K)) for _ in range(2))
times[0] = time.time()
for ε, ε_ in enumerate(εs):
    c_means[ε], c_mses[ε], c_mse_errs[ε], therms[ε] = twodsquareMetropolisCorrelation(
        L, am, ε_, N, int(.1 * N))
    for d in range(2):
        for δ in range(L//2 + 1):
            plt.errorbar(Bs, c_mses[ε, d, δ], yerr=c_mse_errs[ε, d, δ],
                         color=colours[2])
            plt.xscale("log", base=2)
            plt.xlim(1, 2 ** (K-1))
            plt.xlabel(r"$B$")
            plt.ylabel(
                r"$\widehat{\mathrm{MSE}}_{\mathrm{bin}}^{(B)}(\hat{c}_{%s}(%s))$"
                % (d+1, δ))
            plt.title(r"Metropolis, $\epsilon=%s$, $%s\times%s$, $am=%s$, $N=2^{%s}$" %
                      (ε_strings[ε], L, L, am_string, K))
            plt.savefig(
                "%sx%s/Binning (Metropolis, ε = %s, %sx%s, am = %s, N = %s, d = %s, δ = %s).pdf"
                % (L, L, ε_, L, L, am, N, (d+1), δ), bbox_inches="tight")
            plt.show()
    times[ε+1] = time.time()


# Calculating true mean squared errors
c_indexes = np.empty((len(εs), 2, L//2 + 1), dtype=int)
# manually enter indexes here
for d in range(2):
    for δ in range(L//2 + 1):
        c_indexes[4, d, δ] = 8
c_tmses = np.empty((len(εs), 2, L//2 + 1))
for ε in range(len(εs)):
    for d in range(2):
        ...
        #     delimiter=",")
        for δ in range(L//2 + 1):
            c_tmses[ε, d, δ] = c_mses[ε, d, δ, c_indexes[ε, d, δ]]
c_taus = c_tmses / c_mses[:, :, :, 0]
